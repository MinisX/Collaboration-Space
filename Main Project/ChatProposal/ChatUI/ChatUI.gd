			...
			if typeof(json_messages.result) == TYPE_ARRAY:
				for document in json_messages.result:
					if "sender_name" in document and "message" in document:
						...
						_chat_text.append_bbcode("[color=#cccccc]" + document["sender_name"] + "[/color]: " + document["message"])
						_chat_text.newline()
		elif received_data.result["type"] == "message":
			_chat_text.append_bbcode("[color=#cccccc]" + received_data.result["name"] + "[/color]: " + received_data.result["msg"])
			_chat_text.newline()
...
func _on_send() -> void:
	...
	_chat_text.append_bbcode("[color=#cccccc]" + Meeting.participant_data["Name"] + "[/color]: " +_line_edit.text)
	_chat_text.newline()
